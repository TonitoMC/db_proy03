<template>
  <div class="report-container">
    <h2 class="report-title">Leave Analysis Report</h2>
    ...
    <div class="filters-section">
      ...
      <div class="filter-group">
        ...
        <select id="status" v-model="status" class="filter-input select-input">
      ...
      <div class="filter-group">
        <label for="minDuration" class="filter-label"
          >Min Duration (days):</label
        >
        <input
          type="number"
          id="minDuration"
      ...
      <div class="filter-group">
        <label for="maxDuration" class="filter-label"
          >Max Duration (days):</label
        >
        <input
          type="number"
          id="maxDuration"
...
import { fetchLeaveAnalysisReport } from "../api/reportService";
import ReportTable from "../components/ReportTable.vue";
import { exportToCsv, exportTableToPdf } from "../utils/exportUtils";

export default {
  name: "LeaveAnalysisReportView",
  components: {
    ReportTable,
  },
  setup() {
    ...
    const columns = [
      ...
      },
      {
        ...
        label: "End Date",
        formatter: (value) =>
          value ? new Date(value).toLocaleDateString() : "Indefinite",
      },
    ...
    const fetchReport = async () => {
      ...
      if (minDuration.value !== null && minDuration.value !== "")
        filters.min_duration = minDuration.value;
      if (maxDuration.value !== null && maxDuration.value !== "")
        filters.max_duration = maxDuration.value;
      ...
      try {
        const data = await fetchLeaveAnalysisReport(
          startDate.value,
          endDate.value,
          filters,
        );
        if (Array.isArray(data)) {
          ...
          console.warn(
            "Backend returned data for Leave Analysis that is not an array:",
            data,
          );
          reportData.value = [];
        }
      } catch (err) {
        error.value = "Failed to fetch leave analysis report: " + err.message;
        console.error(err);
        reportData.value = [];
      } finally {
        loading.value = false;
    ...
    const handleExportCsv = () => {
      ...
      exportToCsv(columns, reportData.value, "leave_analysis_report.csv");
    };
